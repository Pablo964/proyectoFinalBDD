...
<?php
	...
	$anyomundial = $_REQUEST['anyomundial'];
	$e_l = $_REQUEST['local'];
   ...
   try
   {
	$stid = oci_parse($conexion, 'call prueba.busqPartido(:anyomundial,:eq_l,:eq_v,:sede,:rsPat) into :cantidad');
	$partidos = oci_new_cursor($conexion);
	//$goles = oci_new_cursor($conexion);
	$cantidad=0;
	oci_bind_by_name($stid,":anyomundial",$anyomundial);
